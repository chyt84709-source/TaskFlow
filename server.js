...
const adminUsername = process.env.SUPER_ADMIN_USERNAME || 'Husnain';
const adminPassword = process.env.SUPER_ADMIN_PASSWORD;
if (process.env.NODE_ENV === 'production' && (!process.env.SESSION_SECRET || !adminPassword)) throw new Error('SESSION_SECRET and SUPER_ADMIN_PASSWORD are required in production.');
...
function sendEmail({ to, subject, html }) {
  if (!resend || !process.env.EMAIL_FROM || !to) return Promise.resolve();
...
function issueOtp(phone) { const code = String(crypto.randomInt(100000, 1000000)); const digest = hash(code); if (!reqOtp.has(phone)) reqOtp.set(phone, new Map()); reqOtp.get(phone).set(digest, Date.now() + 5 * 60 * 1000); return code; }
const reqOtp = new Map();

app.get('/api/health', (_req, res) => res.json({ ok: true, service: 'taskflow', time: now() }));
app.get('/api/me', (req, res) => res.json({ user: req.session.user || null }));
app.post('/api/auth/phone/request', (req, res) => { const parsed = z.object({ phone: z.string().min(7).max(20) }).safeParse(req.body); if (!parsed.success) return res.status(400).json({ error: 'Valid phone number required' }); const code = issueOtp(parsed.data.phone); const response = { ok: true, expiresIn: 300 }; if (process.env.NODE_ENV !== 'production') response.developmentCode = code; res.json(response); });
...
seed().then(() => server.listen(port, () => console.log(`TaskFlow listening on http://localhost:${port}`))).catch(error => { console.error('PostgreSQL initialization failed:', error); process.exitCode = 1; });
