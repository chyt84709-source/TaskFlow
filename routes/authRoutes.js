...
router.post('/api/auth/signup', async (req, res, next) => {
  try {
    ...
    const user = { id: nanoid(), email, name: parsed.data.name.trim(), role: parsed.data.role, country: normalizeCountry(parsed.data.country), subscription_tier: parsed.data.subscriptionTier, trust_score: null, two_factor: false, password_hash: passwordHash, created_at: now() };
